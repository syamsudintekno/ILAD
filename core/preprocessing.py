...
def preprocess_dataset(dataset: pd.DataFrame) -> pd.DataFrame:
    ...
    Returns:
        A lecturer-level dataset containing mean indicators, competency
...
def aggregate_by_lecturer(dataset: pd.DataFrame) -> pd.DataFrame:
    """Aggregate EDOM records to one row per lecturer and organizational scope.

    Args:
        dataset: Enriched EDOM records with canonical identity and score columns.

    Returns:
        Mean numeric EDOM data grouped by lecturer, study program, and faculty.

    Raises:
        TypeError: If ``dataset`` is not a pandas DataFrame.
        ValueError: If an identity, indicator, or competency column is absent.
    """
    _ensure_dataframe(dataset)
    score_columns = INDICATOR_COLUMNS + COMPETENCY_COLUMNS + (OVERALL_SCORE_COLUMN,)
    _ensure_columns(dataset, IDENTITY_COLUMNS + score_columns)
    return (
        dataset.groupby(list(IDENTITY_COLUMNS), as_index=False, dropna=False)[
            list(score_columns)
        ]
        .mean()
